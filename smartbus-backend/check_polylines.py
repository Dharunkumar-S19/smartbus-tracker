"""
Check polyline status for all buses.
Shows which buses have polylines and which don't.
"""
...
def check_polylines():
    ...
    if buses_with_polyline:
        ...
        for bus in sorted(buses_with_polyline, key=lambda x: x['id']):
            print(f"  {bus['id']:<12} {bus['name']:<25} {bus['route']:<30} ({bus['points']} points)")
        print()
    ...
    if buses_without_polyline:
        ...
        for bus in sorted(buses_without_polyline, key=lambda x: x['id']):
            print(f"  {bus['id']:<12} {bus['name']:<25} {bus['route']}")
        print()
    ...
    for doc in route_docs:
        ...
        if has_polyline:
            ...
            print(f"  ✅ {route_id:<15} {route_name:<35} {stops_count} stops, {polyline_count} polyline points")
        else:
            print(f"  ❌ {route_id:<15} {route_name:<35} {stops_count} stops, NO POLYLINE")
    ...
    if buses_without_polyline:
        print("⚠️  Some buses don't have polylines!")
        print("   Run: python generate_all_polylines.py")
        print("   Or:  generate_all_polylines.bat")
    else:
        print("🎉 All buses have polylines!")
        print("   Your app should display route lines on the map.")
